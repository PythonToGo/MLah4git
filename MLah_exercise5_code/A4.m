...
t_recData = (0:size(rec, 1)-1)'/fs;

% the noise floor in the time domain
noise_floor = std(rec(:, 2));
% Output the noise floor
disp(['Noise floor in time domain for the total output (u2(t)) is ', num2str(noise_floor), ' V'])
...
figure(41); 
semilogx(fft_freqs, abs(fft_rec(:,2)));
title('Average Spectrum','FontSize',16);
xlabel('Frequency (Hz)','FontSize',14);
ylabel('Magnitude (V)','FontSize',14);

% Plot the outputted signal on channel 1
figure(411);
subplot(211);
plot(t, sig(:,1));
title('Output Signal on Channel 1','FontSize',16);
xlabel('Time (s)','FontSize',14);
ylabel('Amplitude (V)','FontSize',14);

% Plot the recorded u2(t)
subplot(212);
plot(t_recData, rec(:, 2));
title('Recorded Signal u2(t)','FontSize',16);
xlabel('Time (s)','FontSize',14);
ylabel('Amplitude (V)','FontSize',14);
